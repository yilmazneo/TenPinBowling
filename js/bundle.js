...
/***/ "./jsx/TenPinBowlingScoreCalculator.jsx":
/*!**********************************************!*\
  !*** ./jsx/TenPinBowlingScoreCalculator.jsx ***!
  \**********************************************/
/*! no static exports found */
...
var TenPinBowlingScoreCalculator = function (_React$Component) {
    _inherits(TenPinBowlingScoreCalculator, _React$Component);

    function TenPinBowlingScoreCalculator(props) {
        _classCallCheck(this, TenPinBowlingScoreCalculator);

        var _this = _possibleConstructorReturn(this, (TenPinBowlingScoreCalculator.__proto__ || Object.getPrototypeOf(TenPinBowlingScoreCalculator)).call(this, props));

        _this.scoreGame = _this.scoreGame.bind(_this);
        _this.state = { score: 0, isValid: true };
        return _this;
    }

    _createClass(TenPinBowlingScoreCalculator, [{
        key: 'scoreGame',
        value: function scoreGame() {
            var input = document.getElementById('sequenceofrolls');
            var scoreLabel = document.getElementById('scorelabel');
            var points = input.value.split(' ');
            frames = [];
            points.forEach(function (e) {
                var frame = e.split('');frames.push(frame);
            });

            // Parse the input and replace all chars with number
            var parsedFrames = frames.map(function (frame) {
                return frame.map(function (point, i) {
                    if (point === 'X') return 10;else if (point === '/') return 10 - parseInt(frame[i - 1]);else if (point === '-') return 0;else return parseInt(point);
                });
            });
            var isValid = this.validateGame(parsedFrames);
            this.setState({ score: this.calculate(parsedFrames), isValid: isValid });
        }
    }, {
        key: 'validateGame',
        value: function validateGame(frames) {
            var isValid = true;
            var numberOfFrames = frames.length;
            var frameTotals = frames.map(function (v, i) {
                return v.reduce(function (acc, cur, i) {
                    return acc + (i <= 2 ? cur : 0) <= 10;
                });
            });
            var allTotalsInLimit = frameTotals.reduce(function (acc, cur) {
                return acc && cur;
            });
            ...
            // If number of frames is 12 which means 2 bonuses, but the last frame is not strike
                ...
                // If the frame count is 10 and last frame has strike, but bonus points are not there  
                    ...
                    else if (numberOfFrames == 10 && frames[9][0] !== 10 && frames[9][0] + frames[9][1] == 10 && frames[9].length != 3) isValid = false;else if (numberOfFrames < 10) isValid = false;else if (numberOfFrames == 11) isValid = false;else if (numberOfFrames.length > 12) isValid = false;

            return isValid;
        }
    }, {
        key: 'calculate',
        value: function calculate(frames) {
            var _this2 = this;

            var frameTotalsWithBonuses = frames.map(function (v, i) {
                ...
                // if length is 1, it means it is a strike, so add next 2 rolls as bonus
                else if (v.length == 1) return parseInt(v) + parseInt(_this2.getNextTwoRollTotal(frames, i));
                    ...
                    else if (i == 9) return v.reduce(function (acc, cur) {
                            return acc + cur;
                        });
                        // If the total of the frame is 10, then it is a spare, 10 plus the next roll as bonus
                        else if (v.reduce(function (acc, cur) {
                                return acc + cur;
                            }) == 10) return 10 + frames[i + 1][0];
                            // If none of the above caught it, it means it is just a frame that needs to be summed up because total is less than 10
                            else return v.reduce(function (acc, cur) {
                                    return acc + cur;
                                });
            });
            return frameTotalsWithBonuses.reduce(function (acc, cur) {
                return acc + cur;
            });
        }
    }, {
        key: 'getNextTwoRollTotal',
        value: function getNextTwoRollTotal(frames, i) {
            if (frames[i + 1].length == 2) return frames[i + 1].reduce(function (acc, cur) {
                return acc + cur;
        ...
        value: function render() {
            return _react2.default.createElement(
                ...
                { className: 'container' },
                'Sequence Of Rolls:',
                _react2.default.createElement('input', { type: 'text', className: 'rollsinput', id: 'sequenceofrolls' }),
                _react2.default.createElement('input', { type: 'button', onClick: this.scoreGame, className: 'scorebutton', id: 'scorebutton', value: 'Score' }),
                _react2.default.createElement(
                    'div',
                    null,
                    _react2.default.createElement(
                        'h1',
                        { id: 'scorelabel', className: this.state.isValid ? 'visible' : 'hidden' },
                        this.state.score != 0 ? 'Game Score is ' + this.state.score : ''
                    ),
                    _react2.default.createElement(
                        'h1',
                        { id: 'errorlabel', className: this.state.isValid ? 'hidden' : 'visible' },
                        'Invalid Game'
                    )
                )
            );
    ...
    return TenPinBowlingScoreCalculator;
}(_react2.default.Component);

exports.default = TenPinBowlingScoreCalculator;
...
var _TenPinBowlingScoreCalculator = __webpack_require__(/*! ./TenPinBowlingScoreCalculator.jsx */ "./jsx/TenPinBowlingScoreCalculator.jsx");

var _TenPinBowlingScoreCalculator2 = _interopRequireDefault(_TenPinBowlingScoreCalculator);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_reactDom2.default.render(_react2.default.createElement(_TenPinBowlingScoreCalculator2.default, null), document.getElementById('app'));
